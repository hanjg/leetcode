...
public class CourseSchedule {
	public boolean canFinish(int numCourses, int[][] prerequisites) {
		...
		/*
		 * for(int j = 0;j < prerequisites.length;j++){ for(int k = 0;k <
		 * prerequisites[j].length;k++){ if(!dfs(courses,new
		 * boolean[numCourses],prerequisites[j][k])){ return false; } }
		 * 
		 * }
		 */
		for (int j = 0; j < prerequisites.length; j++) {
			boolean[] visited = new boolean[numCourses];
			visited[prerequisites[j][0]] = true;
			if (courses[prerequisites[j][0]][prerequisites[j][1]] != 0) {
				courses[prerequisites[j][0]][prerequisites[j][1]] = 0;
				if (!dfs(courses, visited, prerequisites[j][1]))
					return false;
			}
		}
		return true;
	...
	public boolean dfs(int[][] courses, boolean[] visited, int s) {
		if (visited[s])
			return false;
		visited[s] = true;
		/*
		 * for(int i = 0;i < courses[s].length;i++){ if(courses[s][i] != 0 &&
		 * !dfs(courses,visited,i)){ return false; } }
		 */
		int i = 0;
		for (i = 0; i < courses[s].length; i++) {
			if (courses[s][i] != 0) {
				break;
			}
		}
		if (i == courses[s].length)
			return true;
		else {
			courses[s][i] = 0;
			return dfs(courses, visited, i);
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(cs.canFinish(100,
				new int[][] { { 6, 27 }, { 83, 9 }, { 10, 95 }, { 48, 67 }, { 5, 71 }, { 18, 72 }, { 7, 10 }, { 92, 4 },
						{ 68, 84 }, { 6, 41 }, { 82, 41 }, { 18, 54 }, { 0, 2 }, { 1, 2 }, { 8, 65 }, { 47, 85 },
