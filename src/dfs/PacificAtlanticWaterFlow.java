...
public class PacificAtlanticWaterFlow {
    ...
    public List<int[]> pacificAtlantic(int[][] matrix) {
        res=new ArrayList<>();
    	m=matrix.length;if(m==0)return res;
    	n=matrix[0].length;if(n==0)return res;
    	dirs=new int[][]{{1,0},{-1,0},{0,1},{0,-1}};
        boolean[][]at=new boolean[m][n],pa=new boolean[m][n];//(i,j)是否能流入两个大洋
        for(int i=0;i<m;i++){
        	dfs(matrix, at, i, n-1);
        	dfs(matrix, pa, i, 0);
        }
        ...
        for(int i=0;i<m;i++){
        	for(int j=0;j<n;j++){
        		if(at[i][j]&&pa[i][j])res.add(new int[]{i,j});
        	}
        }
        return res;
    }
